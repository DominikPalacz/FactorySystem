...
export const InboundForm: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: '✅ Przyjęcie zarejestrowane' });
      setLocationId('');
      setItemId('');
      setQuantity('');
      setOperatorId('');
  ...
  return (
    <div className="card space-y-6">
      <h3 className="text-2xl font-bold text-rugged-accent">📥 Przyjęcie Towaru</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {loading ? '⏳ Przetwarzanie...' : '✓ Zatwierdź Przyjęcie'}
        </button>
      </form>
    </div>
  );
